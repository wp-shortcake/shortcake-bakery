...
EOT;
	...
	public function test_https_respect() {
		...
		$post = get_post( $post_id );
		$this->assertContains( 'http://example.org/wp-content/plugins/shortcake-bakery/assets/pdf-viewer/viewer.html?file=' . rawurlencode( 'https://www.gpo.gov/fdsys/pkg/BILLS-114hr2048enr/pdf/BILLS-114hr2048enr.pdf' ), apply_filters( 'the_content', $post->post_content ) );
	}

	public function test_http_respect() {
		$post_id = $this->factory->post->create( array( 'post_content' => '[pdf url="http://www.gpo.gov/fdsys/pkg/BILLS-114hr2048enr/pdf/BILLS-114hr2048enr.pdf"]' ) );
		$post = get_post( $post_id );
		$this->assertContains( 'http://example.org/wp-content/plugins/shortcake-bakery/assets/pdf-viewer/viewer.html?file=' . rawurlencode( 'http://www.gpo.gov/fdsys/pkg/BILLS-114hr2048enr/pdf/BILLS-114hr2048enr.pdf' ), apply_filters( 'the_content', $post->post_content ) );
	}
}
